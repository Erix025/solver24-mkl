...
void exclusive_scan_ilp64(long long int *input, int length)
{
    if (length == 0 || length == 1)
        return;

    long long int old_val, new_val;
...
int read_mtx_header(const char *filename, int *isComplex)
{
    MM_typecode matcode;
    FILE *f;
...
int mmio_allinone(int *m, int *n, int *nnz, int *isSymmetric, int *base, int **csrRowPtr, int **csrColIdx, MAT_VAL_TYPE **csrVal, const char *filename)
{
    int m_tmp, n_tmp;
...
int mmio_allinone_ilp64(long long int *m, long long int *n, long long int *nnz, int *isSymmetric, int *base, long long int **csrRowPtr, long long int **csrColIdx, MAT_VAL_TYPE **csrVal, const char *filename)
{
    int m_tmp, n_tmp;
    long long int nnz_tmp;
    ...
    long long int *csrRowPtr_counter = (long long int *)malloc((m_tmp + 1) * sizeof(long long int));
    memset(csrRowPtr_counter, 0, (m_tmp + 1) * sizeof(long long int));

    long long int *csrRowIdx_tmp = (long long int *)malloc(nnz_mtx_report * sizeof(long long int));
    long long int *csrColIdx_tmp = (long long int *)malloc(nnz_mtx_report * sizeof(long long int));
    MAT_VAL_TYPE *csrVal_tmp = (MAT_VAL_TYPE *)malloc(nnz_mtx_report * sizeof(MAT_VAL_TYPE));
    ...
    for (long long int i = 0; i < nnz_mtx_report; i++)
    {
        int idxi, idxj;
        double fval, fval_im;
        int ival;
    ...
    // exclusive scan for csrRowPtr_counter
    exclusive_scan_ilp64(csrRowPtr_counter, m_tmp + 1);

    long long int *csrRowPtr_alias = (long long int *)malloc((m_tmp + 1) * sizeof(long long int));
    nnz_tmp = csrRowPtr_counter[m_tmp];
    long long int *csrColIdx_alias = (long long int *)malloc(nnz_tmp * sizeof(long long int));
    MAT_VAL_TYPE *csrVal_alias = (MAT_VAL_TYPE *)malloc(nnz_tmp * sizeof(MAT_VAL_TYPE));

    memcpy(csrRowPtr_alias, csrRowPtr_counter, (m_tmp + 1) * sizeof(long long int));
    memset(csrRowPtr_counter, 0, (m_tmp + 1) * sizeof(long long int));
    ...
    if (isSymmetric_tmp)
    {
        for (long long int i = 0; i < nnz_mtx_report; i++)
        {
            if (csrRowIdx_tmp[i] != csrColIdx_tmp[i])
            {
                long long int offset = csrRowPtr_alias[csrRowIdx_tmp[i]] + csrRowPtr_counter[csrRowIdx_tmp[i]];
                csrColIdx_alias[offset] = csrColIdx_tmp[i];
                csrVal_alias[offset] = csrVal_tmp[i];
                csrRowPtr_counter[csrRowIdx_tmp[i]]++;
...
int mmio_allinone_complex(int *m, int *n, int *nnz, int *isSymmetric, int *base, int **csrRowPtr, int **csrColIdx, MAT_VAL_TYPE **csrVal_re,
                          MAT_VAL_TYPE **csrVal_im, const char *filename)
{
    int m_tmp, n_tmp;
    MAT_PTR_TYPE nnz_tmp;
...
int mmio_allinone_complex_ilp64(long long int *m, long long int *n, long long int *nnz, int *isSymmetric, int *base, long long int **csrRowPtr, long long int **csrColIdx, MAT_VAL_TYPE **csrVal_re,
                                MAT_VAL_TYPE **csrVal_im, const char *filename)
{
    int m_tmp, n_tmp;
    int nnz_tmp;
