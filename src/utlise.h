...
int load_vector(int n, double *b, const char *filename);
...
// store x to a file
void store_x(int n, double *x, const char *filename)
{
...
void store_x_complex(int n, double *x, double *x_v, const char *filename)
{
    FILE *p = fopen(filename, "w");
    fprintf(p, "%d\n", n);
    int i;
...
void load_b_complex(int n, double *b, double *b_v, const char *filename)
{
    FILE *p = fopen(filename, "r");
    int n_right;
    // MM_typecode matcode;
