...
void spmv_ld_ilp64(long long int n, long long int *row_ptr, long long int *col_idx, long double *val, long double *x, long double *y)
{
...
void check_correctness_ld_d2ld_ilp64(long long int n, long long int *row_ptr, long long int *col_idx, double *val, double *x, double *b)
{
    //! Step 1: data type transformation: double -> long double
    ...
    long double *check_b = (long double *)malloc(sizeof(long double) * n);
    // long double* r_b     = (long double*)malloc(sizeof(long double) * n);
    spmv_ld_ilp64(n, row_ptr, col_idx, val_ld, x_ld, b_new);
    for (int i = 0; i < n; i++)
    {
        check_b[i] = b_new[i] - b_ld[i];
        // r_b[i]     = fabsl(check_b[i]) / MAX(fabsl(b_ld[i]), 1e-20);
