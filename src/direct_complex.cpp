...
void direct_preprocess_complex(struct DirectComplexSolver *solver, const int n, const int *row_ptr, const int *col_idx)
{
    ...
    // initial
    pardiso(solver->pt, &maxfct, &mnum, &solver->mtype, &solver->phase, &n, NULL, row_ptr, col_idx, NULL, &nrhs, solver->iparm, &msglvl, NULL, NULL,
            &error);
    if (error != 0)
    {
...
void direct_analyze_complex(struct DirectComplexSolver *solver, const int n, const double *val, const double *val_im)
{
    ...
    pardiso(solver->pt, &maxfct, &mnum, &solver->mtype, &solver->phase, &n, (void *)solver->val, solver->row_ptr, solver->col_idx, NULL, &nrhs,
